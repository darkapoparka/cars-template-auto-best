...
- Inventory moves its desktop hero count beside the search prompt. The count is
  derived from the existing inventory filter function and follows pending filters,
  applied filters and zero matches. Its accessible button name includes the count;
  the sticky search repeats it. Mobile/tablet retain their previous presentation.
...
styles; the shared search component owns result-count rendering. No new dependency
...
- `scripts/desktop-routes-smoke.mjs`: 50 BG/EN cases at 390, 992, 1024, 1440 and
  ...
  runtime errors and pending/applied/zero-result counts. Map traffic is cancelled
