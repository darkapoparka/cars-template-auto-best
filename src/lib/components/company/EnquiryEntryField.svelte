<script lang="ts">
  ...
  const summary = $derived(value.trim() ? [value.trim(), kind === 'criteria' && budget ? i18n.t("m_6cf8d9822c2c", { p0: budget }) : ''].filter(Boolean).join(' · ') : '');
  ...
  function save(event: SubmitEvent) {
    ...
    if (kind !== 'criteria' && nextValue) {
      ...
      if (!parsed || (kind === 'listing' && parsed.kind !== 'listing')) {
        ...
        error = kind === 'reference' ? i18n.t("m_4b200427f69a") : i18n.t("m_d0adc258d222");
...
<button {id} class="dn-entry-editor-trigger dn-entry-field" type="button" bind:this={trigger} onclick={() => edit()} aria-haspopup="dialog" aria-controls={`${id}-dialog`} aria-label={`${i18n.text(copy.label)}${summary ? `: ${summary}` : ''}`}>
  <span class:dn-entry-editor-placeholder={!summary}>{summary || i18n.text(copy.placeholder)}</span>
</button>
...
<dialog onkeydown={trapDialogTab} {@attach dialogViewport} id={`${id}-dialog`} class="dn-entry-editor" bind:this={dialog} aria-labelledby={`${id}-title`} onclose={restore} onclick={(event) => { if (event.target === event.currentTarget) dialog.close(); }}>
  ...
  <form bind:this={form} onsubmit={save} novalidate>
    <div class="dn-entry-editor-fields">
      <label class="dn-sr-only" for={`${id}-value`}>{i18n.text(copy.label)}</label>
      {#if kind === 'criteria'}
        <textarea {@attach i18n.validation} id={`${id}-value`} name="entry-value" bind:value={draft} oninput={() => error = ''} maxlength={500} rows="5" placeholder={i18n.t("m_1d58c0cb8a25")}></textarea>
      {:else}
        <input {@attach i18n.validation} id={`${id}-value`} name="entry-value" bind:value={draft} oninput={() => error = ''} maxlength={2048} inputmode={kind === 'listing' ? 'url' : 'text'} autocomplete="off" autocapitalize="none" spellcheck={false} placeholder={i18n.text(copy.placeholder)} aria-invalid={error && errorField === 'entry-value' ? true : undefined} aria-describedby={error && errorField === 'entry-value' ? `${id}-error` : undefined} />
      {/if}
      {#if kind === 'criteria'}
        <label class="dn-entry-editor-budget-label" for={`${id}-budget`}>{i18n.t("m_8acd908fb4a9")} <span>{i18n.t("m_d42086812b73")}</span></label>
        <input {@attach i18n.validation} id={`${id}-budget`} name="entry-budget" bind:value={budgetDraft} oninput={() => error = ''} inputmode="numeric" maxlength={8} placeholder="40000" aria-invalid={error && errorField === 'entry-budget' ? true : undefined} aria-describedby={error && errorField === 'entry-budget' ? `${id}-error` : undefined} />
      {/if}
...
<style>
  ...
  .dn-entry-editor-close { border: 0; border-radius: var(--dn-pill); background: var(--dn-home-panel); color: var(--dn-ink); }
  ...
  input, textarea { display: block; width: 100%; min-height: var(--dn-entry-height); padding: var(--dn-space-2) var(--dn-space-3); border: 1px solid var(--dn-entry-line); border-radius: var(--dn-radius-control); background: var(--dn-entry-surface); color: var(--dn-ink); font: var(--dn-overlay-field-font); }
