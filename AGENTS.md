# Auto Best — AI operating instructions

This repository is the canonical reusable master for the **`auto-best`** automotive template. Treat these instructions as the governing contract for AI work in this repo.
...
Before declaring a lead ready, search the entire copy (including metadata and assets) for old identity markers. Start with: `Auto Best|Day & Night|Day Night|day-night|0877 733 110|087 982 4625|Атанас Манчев`. Treat matches as review items, not blind replacements.
...
- When this template is inserted into a multi-variant dealer repo, it belongs in a variant folder such as `auto-best/` with no nested `.git` directory.
