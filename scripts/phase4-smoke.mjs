...
try {
  await suite.check('URL sort and chip removal retain unrelated filters', async () => {
    ...
    try {
      ...
      await sortSheet.getByRole('button', { name: 'Приложи', exact: true }).click();
      ...
      await page.getByRole('link', { name: 'Премахни BMW', exact: true }).click();
  ...
  await suite.check('nested and outer dialog drafts apply or cancel at their owner', async () => {
    ...
    try {
      ...
      assert.match(await mobileField(page, 'Модел').innerText(), /Всички модели/, 'Changing make must reset model');
      ...
      await trigger.click();
      await dialog.waitFor({ state: 'visible' });
      assert.match(await mobileField(page, 'Марка').innerText(), /Всички марки/, 'Outer Cancel must discard its draft');
      await mobileField(page, 'Марка').click();
      await picker.getByRole('radio', { name: 'BMW', exact: true }).check();
      await picker.getByRole('button', { name: 'Приложи', exact: true }).click();
  ...
  await suite.check('shell state stays correct across direct and client navigation', async () => {
    ...
    try {
      ...
      await page.locator('.dn-mobile-bottom-nav a[href="/listing-grid"]').click();
      ...
      assert.equal(await page.locator('.dn-mobile-bottom-nav a[href="/listing-grid"]').getAttribute('aria-current'), 'page');
      await assertCleanDom(page);
      ...
      assert.match(await page.locator('.dn-mobile-detail-bar__secondary').getAttribute('href'), /^\/contact\?topic=inspection&vehicle=\d+$/);
      ...
      await page.locator('.dn-mobile-bottom-nav a[href="/contact?topic=trade-in"]').click();
      await page.waitForURL(url => url.pathname === '/contact' && url.searchParams.get('topic') === 'trade-in');
      ...
      assert.equal(await page.locator('.dn-mobile-bottom-nav a[href="/contact?topic=trade-in"]').getAttribute('aria-current'), 'page');

      await page.locator('.dn-mobile-bottom-nav a[href="/contact?topic=import"]').click();
      ...
      assert.equal(await page.locator('.dn-mobile-bottom-nav a[href="/contact?topic=import"]').getAttribute('aria-current'), 'page');
      await assertCleanDom(page);
      ...
      await menu.locator('a[href="/about-us"]').click();
      await page.waitForURL(url => url.pathname === '/about-us');
      ...
      await menu.waitFor({ state: 'visible' });
      assert.equal(await menu.locator('a[href="/about-us"]').getAttribute('aria-current'), 'page');
      await page.keyboard.press('Escape');
